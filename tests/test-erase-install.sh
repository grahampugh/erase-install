...
# TEST VERSION OF erase-install THAT IS IDENTICAL BUT DOES NOT PERFORM startosinstall
...
find_existing_installer() {
    ...
    if [[ -f "$macos_dmg" ]]; then
        ...
        hdiutil attach -noverify -quiet "$macos_dmg"
...
# run it! (COMMENTED OUT IN TEST VERSION)
# if [ "$arch" == "arm64" ]; then
